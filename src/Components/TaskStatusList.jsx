...
//         if (parseInt(task.elapsedTime, 10) <= parseInt(task.duration, 10)) {
...
//   const completedTasks = useMemo(() => tasksList.filter((task) => task.completed), [tasksList]);

//   // Memoized daily and weekly data
//   const dailyData = useMemo(() => dailyChartData(completedTasks), [completedTasks]);
//   const weeklyData = useMemo(() => weeklyChartData(completedTasks), [completedTasks]);
...
//         <Bar
//           data={{
//             labels: isSwitch === "Daily" ? dailyData.map((data) => data.name) : weeklyData.map((data) => data.name),
//             datasets: [
//               {
//                 label: "Overdue",
//                 data: isSwitch === "Daily" ? dailyData.map((task) => task.overdue) : weeklyData.map((task) => task.overdue),
//                 backgroundColor: "#EF4444",
//               },
//               {
//                 label: "On Time",
//                 data: isSwitch === "Daily" ? dailyData.map((task) => task.onTime) : weeklyData.map((task) => task.onTime),
...
//         <button
//           className={`py-2 px-2 rounded-md text-sm max-sm:px-1 max-sm:py-1 ${isSwitch === "Daily" ? "bg-black text-white" : "text-black"}`}
//           onClick={() => setSwitch("Daily")}
//         >
//           Daily
//         </button>
//         <button
//           className={`py-2 px-2 rounded-md text-sm max-sm:px-1 max-sm:py-1 ${isSwitch === "Weekly" ? "bg-black text-white" : "text-black"}`}
//           onClick={() => setSwitch("Weekly")}
//         >
//           Weekly
//         </button>
//       </span>
